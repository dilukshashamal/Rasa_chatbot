...
class ActionLLMGenerateResponseBudgetprovided(Action):
    """
    A specific LLM response generation action
    ...
    async def run(
        ...
        # get utter_budget_provided response
        response = select_a_responses(
            domain=domain, name="utter_budget_provided", channel=input_channel
        )
        if not response:
            dispatcher.utter_message(template=DEFAULT_REPHRASE_RESPONSE)
            logger.info(f"Could not retrieve `utter_budget_provided`")
            return []

        llm = OpenAI()
        prompt = OpenAIPrompts.get_generate_prompt(
            template="utter_budget_provided",
            query=latest_query,
            context=response,
            configure_personality=True,
        )
...
class ActionLLMRephraseResponseDeadlineprovided(Action):
    """
    A specific LLM response generation action
    for welcoming the users for the first time.
    """
    ...
    async def run(
        ...
        # define a custom prompt template
        # and placeholders to replace
        template_name = "deadline_provided"
